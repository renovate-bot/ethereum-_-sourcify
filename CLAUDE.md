...
Chain tests (verifying a contract on each supported chain) are not in this repo. They live in
[`sourcifyeth/sourcify-chains`](https://github.com/sourcifyeth/sourcify-chains) alongside the chain
config they exercise.
...
- Chain support is not configured in this repo. The server fetches the chains config at startup from `config.chains.remoteUrl` (`services/server/src/config/default.js`), which points at the file generated by [`sourcifyeth/sourcify-chains`](https://github.com/sourcifyeth/sourcify-chains). Add or change a chain there.
- A local `services/server/src/sourcify-chains.json` file, if present, overrides the remote config entirely (used for self-hosting)
...
- Chain tests for multi-blockchain compatibility live in `sourcifyeth/sourcify-chains`, not here

## Key Configuration Files

- `services/server/src/config/local.js`: Local server configuration
- `services/server/src/config/default.js`: `chains.remoteUrl`, the URL the supported-chains config is fetched from at startup
- `services/server/src/sourcify-chains.json`: Optional local override of the chains config (not committed; replaces the remote config when present)
