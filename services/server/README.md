...
You can run without filling the optional values in `.env` but to connect to some RPCs you need to add API keys as env vars. Check the remote chains config (see [Chains Config](#chains-config)) if the chain you are interested in has an authenticated RPC, or create your own `sourcify-chains.json` file.
...
If there is a `src/sourcify-chains.json` file already, the server will use it. Otherwise, it will fetch the chains config from `chains.remoteUrl` at startup.
...
The chains supported by the Sourcify server are not defined in this repo. At startup the server fetches the chains config from the `chains.remoteUrl` config value (see `src/config/default.js`), which points at the file generated by the [`sourcifyeth/sourcify-chains`](https://github.com/sourcifyeth/sourcify-chains) repo. To add or change a supported chain, open a PR there.

To support a different set of chains, you can create a `src/sourcify-chains.json` file and completely override the remote chains config.
...
```json
{
  ...
  "1": {
    ...
    // optional. If not provided, the rpcs are the ones in the fetched chains config, which sourcifyeth/sourcify-chains generates from chainid.network/chains.json.
    "rpc": [
